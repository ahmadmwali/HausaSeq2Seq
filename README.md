...
The pre-generated dataset used in all paper experiments lives in `./data/supplementary/synthetic_data/`. Use this to reproduce the exact numbers in Tables 2–5.
...
> **Reproducibility disclosure**: when generating the original 450k dataset we did not fix a random seed, so noise insertion was fully stochastic. If you regenerate the data yourself, your metrics will differ slightly from the published tables. The pre-generated files are the authoritative reference.
