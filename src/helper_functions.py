...
def preprocess_function(examples, max_length, model_type):
    """
    Preprocesses the data for Seq2Seq training.
    Maps the 'Noisy' column to inputs and the 'Clean' column to targets.
    """
    # Extract the input and target texts from the dataset batch
    inputs = examples["Noisy"]
    targets = examples["Clean"]

    # We pass text_target=targets to handle target language tokenization automatically,
    # completely bypassing the deprecated .as_target_tokenizer() method.
    model_inputs = tokenizer_instance(
        inputs,
        text_target=targets,
